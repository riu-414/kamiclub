                                        ...
                                        @foreach ($parkings as $parking)
                                            <tr>
                                                ...
                                                <form id="update_{{$parking->id}}" method="post" action="{{ route('admin.parking.update', ['parking' => $parking->id]) }}">
                                                    ...
                                                    @method('PUT')
                                                    <td class="px-4 py-3">
                                                        <a href="#" data-id="{{$parking->id}}" onclick="confirmationPost(this)" class="text-gray bg-gray-300 border-0 py-2 px-8 focus:outline-none hover:bg-gray-400 rounded">変更する</a>
                                                    </td>
                                                </form>
                                            </tr>
                                        @endforeach
